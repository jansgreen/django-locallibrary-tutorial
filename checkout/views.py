...
class checkoutOrder(HttpRequest):
    def index(request):
        number_order = order()
        return render(request, 'checkout_success', number_order)
    ...
    def checkout_process(request):
        ...
        if request.method == 'POST':
            ...
            checkout_order = OrderForm(form_data)
            if checkout_order.is_valid():
                bug = checkout_order.save()
                bug = request.user
        return render(request, 'checkout/checkout_success',form_data)
